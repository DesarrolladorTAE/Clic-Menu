...
import TicketSettingsTabs from "../../../components/operation/ticket/TicketSettingsTabs";
...
export default function TicketSettingsPage() {
  ...
  const canSave = useMemo(() => {
    if (!branchId || !form.paper_width || !form.folio_mode) return false;

    const padding = Number(form.folio_padding);
    if (Number.isNaN(padding) || padding < 1 || padding > 20) return false;
    if (form.folio_mode === "pattern" && !String(form.folio_prefix || "").trim()) return false;
  ...
  const hydrateFormFromResponse = (responseData) => {
    ...
    setForm({
      ...
      whatsapp_contact_phone: setting.whatsapp_contact_phone || "",
      whatsapp_contact_email: setting.whatsapp_contact_email || "",
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      showAlert({
        severity: "error",
        title: "Error",
        message: e?.response?.data?.message || "No se pudo cargar la configuración de tickets.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleChangeField = (field, value) => {
    setForm((prev) => {
      const next = {
        ...prev,
        [field]: field === "folio_padding" ? (value === "" ? "" : Number(value)) : value,
      };

      if (field === "folio_mode" && value === "sequential") next.folio_prefix = "";
      return next;
    });
  };
  ...
  const handleChangePrintField = (field, value) => {
    setPrintForm((prev) => {
      ...
      const selectedOption = printOptions.find(
        (option) =>
          String(option.id) ===
          String(field === "print_app_type_id" ? value : next.print_app_type_id)
      );
      ...
      if (field === "open_drawer" && !value) next.drawer_pin = 0;
  ...
  const handleSavePrint = async () => {
    ...
    try {
      const updated = await updateBranchPrintSetting(restaurantId, branchId, payload);
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      whatsapp_contact_email: form.whatsapp_contact_email?.trim().toLowerCase() || null,
      paper_width: form.paper_width,
      folio_mode: form.folio_mode,
      folio_prefix:
        form.folio_mode === "pattern" ? form.folio_prefix?.trim() || null : null,
  ...
  return (
    <PageContainer>
      <Stack spacing={3}>
        ...
        <TicketSettingsTabs value={activeTab} onChange={setActiveTab} />

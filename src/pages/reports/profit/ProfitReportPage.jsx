...
import ProfitReportHeader from "../../../components/reports/profit/ProfitReportHeader";
import ProfitReportFilters from "../../../components/reports/profit/ProfitReportFilters";
import ProfitReportSummary from "../../../components/reports/profit/ProfitReportSummary";
import ProfitReportPreview from "../../../components/reports/profit/ProfitReportPreview";

import { getBranchesByRestaurant } from "../../../services/restaurant/branch.service";
import {
  getProfitReportSummary,
  getProfitReportTable,
  downloadProfitReportExcel,
  downloadProfitReportPdf,
} from "../../../services/reports/profitReport.service";
...
export default function ProfitReportPage() {
  ...
  const fetchReport = async ({ page = 1, useCurrentFilters = true } = {}) => {
    ...
    try {
      const [summaryResponse, tableResponse] = await Promise.all([
        getProfitReportSummary(restaurantId, paramsBase),
        getProfitReportTable(restaurantId, {
          ...paramsBase,
          page,
          per_page: 15,
        }),
      ...
      showAlert({
        ...
        message:
          e?.response?.data?.message ||
          "No se pudo consultar el reporte de utilidades.",
      });
    } finally {
  ...
  const handleDownloadExcel = async () => {
    ...
    try {
      const response = await downloadProfitReportExcel(
        restaurantId,
        appliedRequestParams
      );

      downloadBlob(response, "reporte_utilidades.xlsx");
    } catch (e) {
      showAlert({
        severity: "error",
        title: "Error",
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      downloadBlob(response, "reporte_utilidades.pdf");
    } catch (e) {
      showAlert({
        severity: "error",
        title: "Error",
        ...
        >
          <Stack spacing={2} alignItems="center">
            ...
            <Typography sx={{ color: "text.secondary", fontSize: 14 }}>
              Cargando reporte de utilidades…
            </Typography>
          </Stack>
        </Box>
      </PageContainer>
  ...
  return (
    <PageContainer>
      <Stack spacing={3}>
        ...
        {hasReport ? (
          <>
            <ProfitReportSummary summary={summary} />

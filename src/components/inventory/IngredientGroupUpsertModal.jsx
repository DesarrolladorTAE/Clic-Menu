...
export default function IngredientGroupUpsertModal({
  open,
  onClose,
  restaurantId,
  editing,
  ...
  const save = async () => {
    ...
    try {
      ...
      if (isEdit) {
        await api.updateIngredientGroup(restaurantId, editing.id, payload);
      } else {
        const res = await api.createIngredientGroup(restaurantId, payload);
      ...
      await onSaved?.({
        ...
        created,
        updatedId: editing?.id || null,
      });
    } catch (e) {
      showAlert({
        severity: "error",
        title: "Error",
        message: normalizeErr(e, "No se pudo guardar el grupo"),
      });
    } finally {
      setSaving(false);
    }
            ...
            <Box>
              ...
              >
                {isEdit
                  ...
                  : "Crea un nuevo grupo para clasificar ingredientes."}
              </Typography>
            </Box>
